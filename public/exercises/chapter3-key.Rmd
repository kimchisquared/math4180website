...
```{r mlb_wins, message=F}
#??mdsr::MLB_teams #to checkout dataset
library(mdsr)
MLB_teams %>% glimpse()
...
MLB_teams %>%
  ...
  facet_wrap(~yearID) +
  labs(title = "Are MLB teams paid accoring to their winnings?",
       subtitle = "Comparing American and National Leagues from 2008-2014",
...
```{r storms, message=F}
library(nasaweather)
#??nasaweather::storms #to check out dataset
...
nasaweather::storms %>%
  ...
  geom_smooth(method = "lm", se=0) +
  facet_wrap(~month) +
  labs(title = "Comparing Monthly Data of Tropical Storms",
       subtitle = "using maximum wind speed and air pressure",
...
```{r railtrail, message=F}
library(mosaicData)
#??mosaicData::RailTrail #to check out data set
mosaicData::RailTrail %>% glimpse()

RailTrail %>% 
  ggplot(aes(hightemp, volume, color = cloudcover)) +
  geom_point() +
  geom_smooth(method ="lm", se=0) +
  facet_wrap(~weekday) +
  labs(title = "What weather conditions do hikers prefer?",
       subtitle = "Comparing max temperature and cloud coverage",
       x = "High Temperature per day (F)",
       y = "Number of hikers per day",
       fill = "Cloud Level (0-10)")
```
...
```{r penguins, message=F, warning=F}
#??palmerpenguins::penguins #to check out dataset
...
penguins %>%
  ...
  geom_smooth(method = "lm", se = 0) +
  facet_wrap(~species) +
  labs(title = "How does bill shape compare across penguin species?",
       subtitle = "Comparing bill length and depth",
       x = "Bill Length (mm)",
       y = "Bill Depth (mm)",
       fill = "Species")
```
...
base_map +
  geom_path(data = storms, aes(color = name, 
                               alpha = 0.1),
